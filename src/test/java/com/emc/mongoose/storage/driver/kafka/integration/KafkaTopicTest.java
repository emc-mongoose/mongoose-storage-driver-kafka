...
//  }
  ...
  public final void testConnectivity()
          throws Exception {
    Thread.sleep(20000);
    try(final var socket = new Socket(kafkaNodeContainer.getContainerIp(), 9092)) {
      assertTrue("Not connected to " + kafkaNodeContainer.getContainerIp() + ":" + 9092, socket.isConnected());
      assertFalse("Closed by server: " + kafkaNodeContainer.getContainerIp() + ":" + 9092, socket.isClosed());
    } catch (Exception e) {

    }
    try(final var socket = new Socket(kafkaNodeContainer.getZookeeperContainerIp(), 2181)) {
      assertTrue("Not connected to " + kafkaNodeContainer.getZookeeperContainerIp() + ":" + 2181, socket.isConnected());
      assertFalse("Closed by server: " + kafkaNodeContainer.getZookeeperContainerIp() + ":" + 2181, socket.isClosed());
  }
